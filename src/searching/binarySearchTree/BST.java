...
public class BST<Key extends Comparable<Key>, Value> {
    ...
    private Node put(Node x, Key key, Value val, int level) {
        ...
        if (x == null) return new Node(key, val, 1, 0, level);
    ...
    /**
     * Prints the keys rooted at that root in level order
     */
    ...
    /**
     * Prints the keys in the subtree rooted at that node in level order
     *
     * @param x root
     */
    ...
    public int levelOrder(Stack<Node> levels) {
        Queue<Node> queue = new Queue<>();
        root.level = 0;
        root.pos = 0;
        queue.enqueue(root);
        ...
        while (!queue.isEmpty()) {
            ...
            maxLevel = Math.max(x.level, maxLevel);
            if (x.left != null) {
    ...
    public void draw() {
        ...
        draw(queue, 1, maxLevel);
    }

    private void draw(Queue<Node> queue, int level, int maxLevel) {
        if (queue.isEmpty() || isAllElementNull(queue)) return;
        ...
        for (int i = 1; i <= endgeLines; i++) {
            for (Node node : queue) {
                ...
                if (node.left != null) StdOut.print("/");
                else printSpaces(1);
                printSpaces(i + i - 1);
                if (node.right != null) StdOut.print("\\");
                else printSpaces(1);
                printSpaces(endgeLines + endgeLines - i);
            }
            StdOut.println("");
        }
        draw(newQueue, level + 1, maxLevel);
    }

    private boolean isAllElementNull(Queue<Node> queue) {
        for (Node node : queue)
    ...
    private class Node {
        ...
        public Node(Key key, Value val, int n, int h, int i) {
            this.key = key;
            this.val = val;
            N = n;
            H = h;
